...
namespace PandoraSharp
{
    public class Crypto
    {
        ...
        public class Blowfish
        {
            ...
            public Blowfish(string key) : this(Encoding.ASCII.GetBytes(key)) { }
            ...
            public void Encrypt(ref byte[] data)
            {
                ...
                if ((data.Length % 8) != 0)
                {
                    byte[] data2 = new byte[data.Length + 8 - (data.Length % 8)];
            ...
            private void EncryptPair(ref uint xl, ref uint xr)
            {
                uint Xl;
                uint Xr;
                uint temp;
            ...
            /// <param name="xl">The left part of the 8 bytes.</param>
            /// <param name="xr">The right part of the 8 bytes.</param>
            private void DecryptPair(ref uint xl, ref uint xr)
            {
                uint Xl;
                uint Xr;
                uint temp;
            ...
            public string Decrypt(string p, bool breakOnNull = true)
            {
                ...
                for (int i = 0; i < p.Length; i += 2)
                    bytes[i / 2] = Convert.ToByte(string.Concat(p[i], p[i + 1]), 16);
            ...
            private string bytesTohex(byte[] bytes)
            {
                ...
                foreach (byte b in bytes)
                {
                    sb.Append(hexmap[(int)((b & 0xf0) >> 4)]);
                    sb.Append(hexmap[(int)(b & 0x0f)]);
                }
