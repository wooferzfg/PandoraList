    ...
    public class Station : INotifyPropertyChanged
    {
        ...
        { 
            ...
            try
            {
                ...
                req["includeCompletePlaylist"] = true;
        ...
        public void AddVariety(SearchResult item)
        {
            ...
            try
            {
                var result = _pandora.CallRPC("station.addMusic", "stationToken", IdToken, "musicToken", item.MusicToken);
            }
        ...
        {
            _pandora.CallRPC("station.deleteMusic", "seedId", seedId);
            RefreshSeeds();
        }
        ...
        public void RefreshSeeds()
        {
            ...
            req2["stationToken"] = IdToken;
            req2["includeExtendedAttributes"] = true;
            var stationInfo = _pandora.CallRPC("station.getStation", req2);
            ParseStationInfo(stationInfo.Result);
        }

        private void ParseStationInfo(JToken d)
        {
            Seeds = new List<Seed>();
            var music = d["music"];
            if (music != null)
            {
                var songs = music["songs"].ToObject<JToken[]>();
                foreach (var song in songs)
                {
                    Seeds.Add(new Seed(song["songName"].ToString(), song["artistName"].ToString(), song["seedId"].ToString()));
                }
                var artists = music["artists"].ToObject<JToken[]>();
                foreach (var artist in artists)
                {
                    Seeds.Add(new Seed(artist["artistName"].ToString(), artist["seedId"].ToString()));
                }
            }
        }
    }
